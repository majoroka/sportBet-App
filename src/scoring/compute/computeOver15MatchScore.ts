...
import {
  OVER_15_MATCH_GROUPS,
  OVER_15_MATCH_PENALTIES,
  PenaltyConfig,
  ScoreGroupConfig,
  ScoreItemConfig,
} from '../models/over15Match.config';
...
const buildItem = (config: ScoreItemConfig, input: Over15MatchInputs): ScoringItem => {
...
const buildGroup = (group: ScoreGroupConfig, input: Over15MatchInputs): ScoringGroup => {
...
export const computeOver15MatchScore = (input: Over15MatchInputs): ScoringResult => {
  const groups = OVER_15_MATCH_GROUPS.map((group) => buildGroup(group, input));
  const { group: penaltyGroup, penaltiesApplied } = buildPenaltyGroup(OVER_15_MATCH_PENALTIES, input, 20);
...
export const createEmptyOver15MatchScore = (reason = 'A carregar...'): ScoringResult => {
  const emptyGroups = OVER_15_MATCH_GROUPS.map((group) => ({
    key: group.key,
    label: group.label,
    points: 0,
