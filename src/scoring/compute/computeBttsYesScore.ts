...
const buildItem = (config: ScoreItemConfig, input: BttsYesInputs): ScoringItem => {
...
const buildGroup = (group: ScoreGroupConfig, input: BttsYesInputs): ScoringGroup => {
...
export const computeBttsYesScore = (input: BttsYesInputs): ScoringResult => {
  const groups = BTTS_YES_GROUPS.map((group) => buildGroup(group, input));
  const { group: penaltyGroup, penaltiesApplied } = buildPenaltyGroup(BTTS_YES_PENALTIES, input, 20);
...
export const createEmptyBttsYesScore = (reason = 'A carregar...'): ScoringResult => {
  const emptyGroups = BTTS_YES_GROUPS.map((group) => ({
    key: group.key,
    label: group.label,
    points: 0,
