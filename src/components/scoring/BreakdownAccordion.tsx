...
const statusClassMap: Record<ScoringStatus, { dot: string; text: string }> = {
  ...
  neutral: { dot: 'bg-gray-300', text: 'text-gray-400' },
};
    ...
    <div className="mt-4">
      ...
      {open && (
        <div className="mt-3 space-y-3">
          {groups.map((group) => (
            <div key={group.key} className="rounded-md border border-gray-200 bg-gray-50/60 p-3">
              <div className="flex items-center justify-between text-xs font-semibold uppercase text-gray-600">
              ...
              <div className="mt-2 space-y-1">
                {group.items.map((item) => {
                  ...
                  return (
                    <div key={item.key} className="flex items-center justify-between text-sm text-gray-700">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={`h-2 w-2 rounded-full ${classes.dot}`} />
                        <span className="truncate" title={item.label}>{item.label}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500">{getValueLabel(item)}</span>
